    ...
    void printVertex(void)
      {
        ...
        for (int i=0; i<degree; i++) printf("%d ",(*nbrs)[i]);
        printf(")\n");
      }
    ...
    void removeNbr(int y)
      {
        ...
        //nous sommes censés être connectés au sommet y et devons le supprimer
        if (i<degree)
          {
  ...
  public:
    ...
    Graph(int x)
      { 
        ...
        // Définition de l'énumération consécutive et de l'ordre des sommets par défaut, juste au cas où
    ...
    void printClique (deque<Vertex> * g)
      {
        int sz = g->size();
        printf ("Clique size %d is: ( ",sz);
        for (int j=0; j<sz; j++) printf("%d ",(*g)[j].getV());
        printf(")\n");
      }
      ...
      for (const auto & [key, value] : *cliques)
        {
          sz = value.size();
          printf ("Clique size %d is: ( ",sz);
            for (int j=0; j<sz; j++) printf("%d ",value[j]);
          printf(")\n");
        }
    }
...
void usage (void)
{
  printf("Usage: ./codes -f file_with_graph_edges_TAB_delimited [-a algorithm_number 0..2] [-s vertex__with_smallest_number]\n");
  printf("By default it uses generic clique search, vertex numeration starting from zero\n");
}
...
int main(int argc, char **argv)
{
  ...
  switch (alg_sel)
    {
      case 0:
        printf("Generic search for cliques using degeneracy order\n");
        gg.Bron_KerboschDegenBuild();
        break;
      case 1:
        printf("Algorithm 1 for cliques using degeneracy order\n");
        gg.Alg1Build();
        gg.printCliques();
        break;
      case 2:
        printf("Algorithm 2 for cliques using degeneracy order\n");
        gg.Alg2Build();
        break;
      default:
        printf("Invalid selection of algorithm.\n");
        return(1);
    }
  printf("Overall number of recursive calls: %ld\n",gg.getRcalls());
  return 0;
}
