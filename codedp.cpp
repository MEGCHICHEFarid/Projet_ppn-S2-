  ...
  public:
    ...
    Graph(int x)
      { 
        ...
        // Setting consequential enumeration and default vertex order, just in case
...
//            #pragma omp parallel for private (i,k,cnt)
            for (i=0;i<R->size();i++)
            {
              localNb = (*R)[i].getNbrs();
...
void usage (void)
{
  printf("Usage: ./codep -f file_with_graph_edges_TAB_delimited [-a algorithm_number 0..2] [-s vertex__with_smallest_number] [-t number_of_threads]\n");
  printf("By default it uses generic clique search, vertex numeration starting from zero and 2 parallel threads\n");
}
